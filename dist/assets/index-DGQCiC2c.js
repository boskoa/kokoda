import{d as s,r,j as e,m as ae,k as C,a as R,s as ce,u as le,c as Ce,G as Se,l as $e,g as de,f as Ae,n as pe,o as L,p as Ie,q as Me,t as Re,v as Be,W as Ee,I as Le,w as ze,x as De,y as Te,N as Fe}from"./index-CoZiDz3r.js";const Ne=s.div`
  position: fixed;
  bottom: 0px;
  width: inherit;
  display: flex;
...
`,bt=s.p`
  overflow-anchor: auto;
  height: 1px;
`;function wt(){const{id:n}=Me(),c=R(ce),t=R(a=>Re(a,n)),i=R(pe),[p,g]=r.useState([]),[m,x]=r.useState(!1),f=de(),S=20,[j,z]=r.useState(!1),v=R(a=>Be(a,n)),[Y,k]=r.useState(!1),[D,T]=r.useState(!1),[X,q]=r.useState(!1),J=r.useRef(null),$=r.useRef(0),B=r.useRef(null),A=r.useRef(!1),F=r.useRef(null),N=r.useRef(!0),I=Ye(F,document.getElementById("vp")),h=r.useContext(Ee),G=r.useCallback(async a=>{const{token:l,id:u,offset:d,limit:O}=a,Z={headers:{Authorization:`bearer ${l}`}},E=await C.get(`/api/messages/chat/${u}?pagination=${d},${O}`,Z);if(z(!1),E.data.length!==O&&(A.current=!0),!E.data.length){A.current=!0;return}g(H=>H.length?[...H,...E.data]:[...E.data])},[]);if(r.useEffect(()=>{z(!0),I&&!A.current&&(G({token:c.token,id:n,offset:$.current,limit:S}),$.current+=S)},[I,c,n,G]),r.useEffect(()=>{if(z(!1),B.current&&(B.current.childNodes.forEach(l=>l.classList.remove("date")),Object.keys(Object.groupBy(p,({createdAt:l})=>new Date(l).toLocaleString("en-GB").slice(0,10))).forEach(l=>{const u=document.querySelectorAll(`[data-date="${l}"]`);u[u.length-1].classList.add("date")})),N.current&&p.length){const a=document.getElementById("vp");a.scrollTop=a.scrollHeight,N.current=!1}},[p]),r.useEffect(()=>{!N.current&&h&&h.chatId==n&&(g(a=>a.length?a.map(l=>l.id).includes(h.id)?a.map(l=>l.id===h.id?h:l):[h,...a.filter(l=>l.id!==h.id)]:[h]),$.current+=1)},[h,n,c]),r.useLayoutEffect(()=>{const a=document.getElementById("vp");let l=0;function u(d){d.target.scrollTop<100&&(d.target.scrollTop=100),d.target.scrollHeight-d.target.scrollTop<d.target.offsetHeight+50&&f(Te({token:c.token,count:0,chatId:n})),d.target.scrollHeight-d.target.scrollTop>d.target.offsetHeight+500?d.target.scrollTop-l>10&&d.target.scrollTop-l<500?k(!0):d.target.scrollTop<l&&k(!1):k(!1),l=d.target.scrollTop}return a.addEventListener("scroll",u),()=>{a.removeEventListener("scroll",u)}},[]),!t||!i||i.find(a=>a.id===t.members.find(l=>l!==c.id))===void 0)return e.jsx(K,{endRef:F,loading:I&&j&&!A.current});async function w(a){const l={headers:{Authorization:`bearer ${c.token}`}};try{await C.post("/api/messages",{chatId:t.id,text:a},l)}catch(u){u.response.data.error==="You are blocked by this user"&&x(!0)}}return console.log("USERS",i),e.jsxs(xt,{$backgroundUrl:X?"":`/public/uploads/backgrounds/${c==null?void 0:c.id}-${t==null?void 0:t.id}.webp`,children:[e.jsxs(gt,{children:[e.jsx(Le.Provider,{value:{color:"gold",size:"2em"},children:e.jsx(ft,{to:"/chats",title:"Go back",children:e.jsx(ze,{})})}),t.name||i.find(a=>a.id===t.members.find(l=>l!==c.id)).name,e.jsx(ht,{onClick:()=>T(a=>!a),children:e.jsx("img",{src:De,title:"Chat settings",style:{filter:"brightness(0) saturate(100%) invert(91%) sepia(17%) saturate(4935%) hue-rotate(357deg) brightness(99%) contrast(105%)"}})})]}),e.jsxs(mt,{ref:B,$limit:S,children:[e.jsx(bt,{}),p.map(a=>{var l;return e.jsxs(et,{parentWidth:(l=B.current)==null?void 0:l.offsetWidth,message:a,setMessages:g,children:[a.id,": ",a.text]},a.id)}),e.jsx(K,{endRef:F,loading:I&&j&&!A.current,style:{marginBottom:100}})]}),e.jsx(He,{send:w,blocked:m}),e.jsx(it,{unseen:v==null?void 0:v.count,scrollDown:Y}),e.jsx(ut,{ref:J,show:D,chat:t,loggedUser:c,setDeletedBg:q})]})}export{wt as default};
