import{d as s,r,j as e,m as ae,k as C,a as B,s as ce,u as le,c as Ce,G as Se,l as $e,g as de,f as Ae,n as pe,o as T,p as Ie,q as Me,t as Be,v as Ee,W as Le,I as Re,w as Te,x as ze,y as De,N as Fe}from"./index-B46HOFuL.js";const Ne=s.div`
  position: fixed;
  bottom: 0px;
  width: inherit;
  display: flex;
...
`,bt=s.p`
  overflow-anchor: auto;
  height: 1px;
`;function jt(){const{id:n}=Me(),c=B(ce),t=B(a=>Be(a,n)),i=B(pe),[p,g]=r.useState([]),[m,x]=r.useState(!1),f=de(),S=20,[k,z]=r.useState(!1),v=B(a=>Ee(a,n)),[Y,w]=r.useState(!1),[D,F]=r.useState(!1),[X,q]=r.useState(!1),J=r.useRef(null),$=r.useRef(0),E=r.useRef(null),A=r.useRef(!1),N=r.useRef(null),L=r.useRef(!0),I=Ye(N,document.getElementById("vp")),h=r.useContext(Le),G=r.useCallback(async a=>{const{token:l,id:u,offset:d,limit:O}=a,Z={headers:{Authorization:`bearer ${l}`}},R=await C.get(`/api/messages/chat/${u}?pagination=${d},${O}`,Z);if(z(!1),R.data.length!==O&&(A.current=!0),!R.data.length){A.current=!0;return}g(H=>H.length?[...H,...R.data]:[...R.data])},[]);if(r.useEffect(()=>{z(!0),I&&!A.current&&(G({token:c.token,id:n,offset:$.current,limit:S}),$.current+=S)},[I,c,n,G]),r.useEffect(()=>{if(z(!1),E.current&&(E.current.childNodes.forEach(l=>l.classList.remove("date")),Object.keys(Object.groupBy(p,({createdAt:l})=>new Date(l).toLocaleString("en-GB").slice(0,10))).forEach(l=>{const u=document.querySelectorAll(`[data-date="${l}"]`);u[u.length-1].classList.add("date")})),L.current&&p.length){const a=document.getElementById("vp");setTimeout(()=>a.scrollTop=a.scrollHeight,100),L.current=!1}},[p]),r.useEffect(()=>{let a=setTimeout(()=>L.current=!1,1e3);return()=>clearTimeout(a)},[]),r.useEffect(()=>{!L.current&&h&&h.chatId==n&&(g(a=>a.length?a.map(l=>l.id).includes(h.id)?a.map(l=>l.id===h.id?h:l):[h,...a.filter(l=>l.id!==h.id)]:[h]),$.current+=1)},[h,n,c]),r.useLayoutEffect(()=>{const a=document.getElementById("vp");let l=0;function u(d){d.target.scrollTop<100&&(d.target.scrollTop=100),d.target.scrollHeight-d.target.scrollTop<d.target.offsetHeight+50&&f(De({token:c.token,count:0,chatId:n})),d.target.scrollHeight-d.target.scrollTop>d.target.offsetHeight+500?d.target.scrollTop-l>10&&d.target.scrollTop-l<500?w(!0):d.target.scrollTop<l&&w(!1):w(!1),l=d.target.scrollTop}return a.addEventListener("scroll",u),()=>{a.removeEventListener("scroll",u)}},[]),!t||!i||i.find(a=>a.id===t.members.find(l=>l!==c.id))===void 0)return e.jsx(K,{endRef:N,loading:I&&k&&!A.current});async function j(a){const l={headers:{Authorization:`bearer ${c.token}`}};try{await C.post("/api/messages",{chatId:t.id,text:a},l)}catch(u){u.response.data.error==="You are blocked by this user"&&x(!0)}}return e.jsxs(xt,{$backgroundUrl:X?"":`/public/uploads/backgrounds/${c==null?void 0:c.id}-${t==null?void 0:t.id}.webp`,children:[e.jsxs(gt,{children:[e.jsx(Re.Provider,{value:{color:"gold",size:"2em"},children:e.jsx(ft,{to:"/chats",title:"Go back",children:e.jsx(Te,{})})}),t.name||i.find(a=>a.id===t.members.find(l=>l!==c.id)).name,e.jsx(ht,{onClick:()=>F(a=>!a),children:e.jsx("img",{src:ze,title:"Chat settings",style:{filter:"brightness(0) saturate(100%) invert(91%) sepia(17%) saturate(4935%) hue-rotate(357deg) brightness(99%) contrast(105%)"}})})]}),e.jsxs(mt,{ref:E,$limit:S,children:[e.jsx(bt,{}),p.map(a=>{var l;return e.jsxs(et,{parentWidth:(l=E.current)==null?void 0:l.offsetWidth,message:a,setMessages:g,children:[a.id,": ",a.text]},a.id)}),e.jsx(K,{endRef:N,loading:I&&k&&!A.current,style:{marginBottom:100}})]}),e.jsx(He,{send:j,blocked:m}),e.jsx(it,{unseen:v==null?void 0:v.count,scrollDown:Y}),e.jsx(ut,{ref:J,show:D,chat:t,loggedUser:c,setDeletedBg:q})]})}export{jt as default};
