import{d as s,r,j as e,m as ae,k as C,a as B,s as ce,u as le,c as Ce,G as Se,l as $e,g as de,f as Ae,n as pe,o as z,p as Ie,q as Me,t as Be,v as Le,W as Re,I as ze,w as Ee,x as De,y as Te,N as Fe}from"./index-uHRheYgX.js";const Ne=s.div`
  position: fixed;
  bottom: 0px;
  width: inherit;
  display: flex;
...
`,bt=s.p`
  overflow-anchor: auto;
  height: 1px;
`;function wt(){const{id:n}=Me(),c=B(ce),t=B(l=>Be(l,n)),i=B(pe),[p,g]=r.useState([]),[m,x]=r.useState(!1),f=de(),S=20,[j,E]=r.useState(!1),v=B(l=>Le(l,n)),[Y,k]=r.useState(!1),[D,T]=r.useState(!1),[X,q]=r.useState(!1),J=r.useRef(null),$=r.useRef(0),L=r.useRef(null),A=r.useRef(!1),F=r.useRef(null),N=r.useRef(!0),I=Ye(F,document.getElementById("vp")),h=r.useContext(Re),G=r.useCallback(async l=>{const{token:a,id:u,offset:d,limit:O}=l,Z={headers:{Authorization:`bearer ${a}`}},R=await C.get(`/api/messages/chat/${u}?pagination=${d},${O}`,Z);if(E(!1),R.data.length!==O&&(A.current=!0),!R.data.length){A.current=!0;return}g(H=>H.length?[...H,...R.data]:[...R.data])},[]);if(r.useEffect(()=>{E(!0),I&&!A.current&&(G({token:c.token,id:n,offset:$.current,limit:S}),$.current+=S)},[I,c,n,G]),r.useEffect(()=>{if(E(!1),L.current&&L.current.childNodes.forEach(a=>a.classList.remove("date")),Object.keys(Object.groupBy(p,({createdAt:a})=>new Date(a).toLocaleString("en-GB").slice(0,10))).forEach(a=>{const u=document.querySelectorAll(`[data-date="${a}"]`);u[u.length-1].classList.add("date")}),N.current&&p.length){const a=document.getElementById("vp");a.scrollTop=a.scrollHeight,N.current=!1}},[p]),r.useEffect(()=>{!N.current&&h&&h.chatId==n&&(g(l=>l.length?l.map(a=>a.id).includes(h.id)?l.map(a=>a.id===h.id?h:a):[h,...l.filter(a=>a.id!==h.id)]:[h]),$.current+=1)},[h,n,c]),r.useLayoutEffect(()=>{const l=document.getElementById("vp");let a=0;function u(d){d.target.scrollTop<100&&(d.target.scrollTop=100),d.target.scrollHeight-d.target.scrollTop<d.target.offsetHeight+50&&f(Te({token:c.token,count:0,chatId:n})),d.target.scrollHeight-d.target.scrollTop>d.target.offsetHeight+500?d.target.scrollTop-a>10&&d.target.scrollTop-a<500?k(!0):d.target.scrollTop<a&&k(!1):k(!1),a=d.target.scrollTop}return l.addEventListener("scroll",u),()=>{l.removeEventListener("scroll",u)}},[]),!t||!i)return e.jsx(K,{endRef:F,loading:I&&j&&!A.current});async function w(l){const a={headers:{Authorization:`bearer ${c.token}`}};try{await C.post("/api/messages",{chatId:t.id,text:l},a)}catch(u){u.response.data.error==="You are blocked by this user"&&x(!0)}}return e.jsxs(xt,{$backgroundUrl:X?"":`/public/uploads/backgrounds/${c==null?void 0:c.id}-${t==null?void 0:t.id}.webp`,children:[e.jsxs(gt,{children:[e.jsx(ze.Provider,{value:{color:"gold",size:"2em"},children:e.jsx(ft,{to:"/chats",title:"Go back",children:e.jsx(Ee,{})})}),t.name||i.find(l=>l.id===t.members.find(a=>a!==c.id)).name,e.jsx(ht,{onClick:()=>T(l=>!l),children:e.jsx("img",{src:De,title:"Chat settings",style:{filter:"brightness(0) saturate(100%) invert(91%) sepia(17%) saturate(4935%) hue-rotate(357deg) brightness(99%) contrast(105%)"}})})]}),e.jsxs(mt,{ref:L,$limit:S,children:[e.jsx(bt,{}),p.map(l=>{var a;return e.jsxs(et,{parentWidth:(a=L.current)==null?void 0:a.offsetWidth,message:l,setMessages:g,children:[l.id,": ",l.text]},l.id)}),e.jsx(K,{endRef:F,loading:I&&j&&!A.current,style:{marginBottom:100}})]}),e.jsx(He,{send:w,blocked:m}),e.jsx(it,{unseen:v==null?void 0:v.count,scrollDown:Y}),e.jsx(ut,{ref:J,show:D,chat:t,loggedUser:c,setDeletedBg:q})]})}export{wt as default};
